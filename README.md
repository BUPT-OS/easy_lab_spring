...
有关日志系统的更多内容，同学们可以认真阅读 **Linux 内核文档 [3]**，该文档中对日志数据结构做了详细描述，参考资料 **[4]** 中也涵盖了一部分。
...
和基础版实验相同，本次实验会为同学们每人制作一份不同的 Ext3 文件系统镜像，为了减小实验难度，文件系统镜像的制作方式是：创建镜像、大量随机写入新文件、写入目标文件、大量随机写入新文件、删除目标文件。同学们使用实现的代码，从镜像中各自恢复出属于自己的文件内容（文件内部均是UTF-8编码格式的英文字母），然后提交到测试平台即可，后台会自动匹配内容来判断是否通过测试。
